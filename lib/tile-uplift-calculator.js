...
const EXPOSURE_PARAMS = {
  // ASCE 7 power law parameters commonly used for velocity pressure coefficient calculations.
  C: { alpha: 9.5, zg: 900 },
  D: { alpha: 11.5, zg: 700 },
};
...
export function calculateKz(h, exposure) {
  ...
  if (!exposureParams) {
    throw new Error('Exposure must be C or D.');
  }
...
export function calculateTileUpliftMoment({
  ...
  // Rigid tile uplift check commonly takes moment arm as half tile width.
  const La = b / 2;
